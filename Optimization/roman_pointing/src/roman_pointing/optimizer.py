...
5. EXOSIMS SESSION
   - TargetList is instantiated ONCE per pipeline run via CorgietcSession.
   - We mutate mode["TimeonRefStar_tRef_per_tTar"] and mode["RefStar_V_mag"]
     in-place between calls, exactly as notebook 07 does.
   - NO cache-wiping, NO importlib.reload() between targets — those break
     EXOSIMS's internal cross-references and cause OS to come back as a module
...
REF_FRACS = np.linspace(0.01, 1.0, 100)   # R sweep grid
...
class CorgietcSession:
    """
    Holds a single live EXOSIMS TargetList + OpticalSystem + TimeKeeping.

    Usage
    -----
    session = CorgietcSession(band=1)          # one-time init
    t_sci   = session.calc_inttime(WA, dMag, R=0.3, ref_vmag=5.1)

    Call session.set_science_star(sy_vmag) before looping over epochs
    for a new science target.  Call session.set_ref_star(vmag, spectype)
    when the reference star changes.
    """

    def __init__(self, band: int, months_at_l2: float = 21):
        import EXOSIMS.Prototypes.TargetList as _TLmod
        import EXOSIMS.Prototypes.TimeKeeping as _TKmod
        ...
        self.TL = _TLmod.TargetList(**copy.deepcopy(specs))
        self.OS = self.TL.OpticalSystem

        # Guard: make sure we got an object, not a module
        if not hasattr(self.OS, "calc_intTime") or not callable(self.OS.calc_intTime):
            raise RuntimeError(
                f"EXOSIMS OpticalSystem failed to instantiate — "
                f"OS is {type(self.OS)}, not an object. "
                "Check that all EXOSIMS imports completed cleanly."
            )

        self.TK = _TKmod.TimeKeeping(missionLife=5.25)
        self.TK.allocate_time(months_at_l2 * 30.4375 * u.d)
        self.fZ = np.repeat(self.TL.ZodiacalLight.fZ0, 1)

        target_nm = BAND_WAVELENGTH[band]
        self.mode = next(
            (m for m in self.OS.observingModes
             if abs(m["lam"].to_value(u.nm) - target_nm) < 20),
            self.OS.observingModes[0],
        )
        self.band = band

        # Remember default Vmag so we can scale fluxes per science star
        self._vmag_default  = float(self.TL.Vmag[0])
        self._orig_vmag     = self.TL.Vmag.copy()
        self._orig_fluxes   = {
            m["hex"]: self.TL.star_fluxes[m["hex"]].copy()
            for m in self.OS.observingModes
            if m["hex"] in self.TL.star_fluxes
        }
        self._current_sy_vmag = self._vmag_default

    # ------------------------------------------------------------------
    def set_science_star(self, sy_vmag: float) -> None:
        """
        Scale star fluxes so the session represents a science star of
        V-magnitude sy_vmag. Call once per science target before looping
        over epochs.
        """
        if sy_vmag == self._current_sy_vmag:
            return
        flux_scale = 10.0 ** ((self._vmag_default - sy_vmag) / 2.5)
        for m in self.OS.observingModes:
            hk = m["hex"]
            if hk in self._orig_fluxes:
                self.TL.star_fluxes[hk] = self._orig_fluxes[hk] * flux_scale
        self.TL.Vmag[0] = sy_vmag
        self._current_sy_vmag = sy_vmag

    def restore_science_star(self) -> None:
        """Undo set_science_star() — not needed unless you want a clean state."""
        for hk, orig in self._orig_fluxes.items():
            self.TL.star_fluxes[hk] = orig
        self.TL.Vmag[0] = self._orig_vmag[0]
        self._current_sy_vmag = self._vmag_default

    # ------------------------------------------------------------------
    def set_ref_star(self, ref_vmag: float, ref_spectype: str = "a0v") -> None:
        """Mutate mode in-place — exactly what notebook 07 does."""
        self.mode["RefStar_V_mag"]        = ref_vmag
        self.mode["RefStar_SpectralType"] = ref_spectype

    def set_R(self, R: float) -> None:
        """Set tRef_per_tTar for next calc_intTime call."""
        self.mode["TimeonRefStar_tRef_per_tTar"] = R

    # ------------------------------------------------------------------
    def calc_inttime_hours(
        self,
        WA_arcsec: float,
        dMag: float,
    ) -> float:
        """
        Return T_science in hours for one (WA, dMag) epoch.

        Assumes set_science_star(), set_ref_star(), and set_R() have already
        been called. Returns NaN if outside IWA/OWA or calc_intTime returns
        a non-positive result.
        """
        WA = np.array([WA_arcsec]) * u.arcsec
        if (WA[0] < self.mode["IWA"]) or (WA[0] > self.mode["OWA"]):
            return np.nan
        JEZ = self.TL.JEZ0[self.mode["hex"]] / (WA_arcsec ** 2)
        t_hr = self.OS.calc_intTime(
            self.TL, [0], self.fZ, JEZ,
            np.array([dMag]), WA, self.mode, TK=self.TK,
        )[0].to(u.hour).value
        return t_hr if (np.isfinite(t_hr) and t_hr > 0) else np.nan
    ...
    def sweep_R(
        ...
        """
        Sweep REF_FRACS at one (WA, dMag) epoch.
        Returns totTimes_s = (1+R) * intTime_s for each R in REF_FRACS.
        NaN where outside IWA/OWA or calc_intTime returns garbage.
        """
        WA = np.array([WA_arcsec]) * u.arcsec
        if (WA[0] < self.mode["IWA"]) or (WA[0] > self.mode["OWA"]):
        ...
        for j, R in enumerate(REF_FRACS):
            self.mode["TimeonRefStar_tRef_per_tTar"] = R
            intTimes[j] = self.OS.calc_intTime(
                self.TL, [0], self.fZ, JEZ, dMag_arr, WA, self.mode, TK=self.TK,
            )[0]

        totTimes_s = (intTimes * (1.0 + REF_FRACS)).to(u.s).value
        totTimes_s[~np.isfinite(totTimes_s) | (totTimes_s <= 0)] = np.nan
        return totTimes_s


# Module-level session cache: band → CorgietcSession
# Populated lazily so we don't pay init cost unless engine is provided.
_SESSIONS: dict[int, CorgietcSession] = {}

def _get_session(band: int, months_at_l2: float = 21) -> CorgietcSession:
    """Return (or create) the singleton session for this band."""
    if band not in _SESSIONS:
        _SESSIONS[band] = CorgietcSession(band, months_at_l2)
    return _SESSIONS[band]
...
def compute_wall_clock_s(
    ...
    """
    Authoritative wall-clock formula.

    T_wall = Σ_i (1 + R_i) × T_science_i
           + T_slew_inter
           + Σ_i N_slew × T_slew_sci_to_ref_i
           + n_science × N_rollchange × T_rollchange
           + n_science × T_HOWFSC
    """
    ...
    intra_slew_s = sum(adi["N_slew"] * intra_slew[k] for k in sci_keys)
...
# ===========================================================================
# SECTION 3 — SPECTRAL TYPE + SIMBAD UTILITIES
# ===========================================================================
...
def _get_ref_spectype(ref_star_name: str | None) -> str:
    """Fetch spectral type of a reference star from SIMBAD, mapped to BPGS."""
...
def _fetch_orbit_df(pl_name: str, band: int, engine) -> pd.DataFrame:
    """Fetch orbit+star data from plandb for one planet/band."""
...
# ===========================================================================
# SECTION 4 — INTEGRATION TIME WITH ACTUAL REF STAR
# ===========================================================================
...
def compute_inttime_with_ref_star(
    ...
    Uses the singleton CorgietcSession — no re-instantiation of TargetList.

    Returns dict with keys:
        t, WA_arcsec, dMag, intTime_hours, t_wall_hours,
        ref_star, ref_star_vmag, R, source
    """
    ref_star_vmag = get_ref_star_vmag(ref_star_name) if ref_star_name else 2.26
    ref_spectype  = (
        _normalize_spectype(ref_star_spectype)
        if ref_star_spectype else _get_ref_spectype(ref_star_name)
    )

    df = _fetch_orbit_df(pl_name, band, engine)
    if df.empty:
        raise ValueError(f"No orbit data for '{pl_name}' band {band}")

    sy_vmag = float(df["sy_vmag"].iloc[0])

    session = _get_session(band, months_at_l2)
    session.set_science_star(sy_vmag)
    session.set_ref_star(ref_star_vmag, ref_spectype)
    session.set_R(R)
    ...
    for i, row in df.iterrows():
        if np.isnan(row["dMag"]) or np.isnan(row["WA"]):
            continue
        intTimes_hours[i] = session.calc_inttime_hours(float(row["WA"]), float(row["dMag"]))
...
def compare_ref_star_impact(
    ...
    Runs two calc_intTime sweeps (default vs actual) using the singleton session.
    """
    ...
    print(f"  Default vmag   : 2.26  R_default={R_default}")
    print(f"  Actual  vmag   : {actual_vmag:.2f}  R_actual={R_actual}")
    print(f"{'='*65}")
    ...
    sy_vmag = float(df["sy_vmag"].iloc[0])
    session = _get_session(band)
    session.set_science_star(sy_vmag)

    def _run(ref_vmag: float, R_val: float) -> np.ndarray:
        session.set_ref_star(ref_vmag)
        session.set_R(R_val)
        results = []
        for _, row in df.iterrows():
            if np.isnan(row["dMag"]) or np.isnan(row["WA"]):
                ...
                results.append(session.calc_inttime_hours(float(row["WA"]), float(row["dMag"])))
...
def optimise_R(
    ...
    Uses the singleton CorgietcSession — no TargetList re-construction.

    Returns dict with R_opt, T_sci_opt_hrs, T_wall_opt_hrs, saving_pct, etc.
    """
    ref_star_vmag = get_ref_star_vmag(ref_star_name) if ref_star_name else 2.26
    ref_spectype  = (
        _normalize_spectype(ref_star_spectype)
    ...
    if verbose:
        ...
        print(f"    ref={ref_star_name or 'none'}  ref_vmag={ref_star_vmag:.2f}  spectype={ref_spectype}")
    ...
    _fallback = {
        ...
        "ref_star_vmag": ref_star_vmag, "ref_star_spectype": ref_spectype,
    ...
    # Set up session once for this target
    session = _get_session(band, months_at_l2)
    session.set_science_star(sy_vmag)
    session.set_ref_star(ref_star_vmag, ref_spectype)
    ...
    for i, row in df_obs.iterrows():
        T_wall_all[i] = session.sweep_R(float(row["WA"]), float(row["dMag"]))
        if verbose and (i % max(1, n // 5) == 0):
            print(f"    epoch {i+1}/{n}  WA={row['WA']:.3f}\"  dMag={row['dMag']:.1f}", end="\r")
...
# ===========================================================================
# SECTION 6 — PLANET / TARGET DATA UTILITIES
# ===========================================================================
...
def get_target_tint_entry(
    ...
    Priority:
        1. engine + pl_name → compute_inttime_with_ref_star() (corrected T_science)
        2. Otherwise → median from .npy files with R_default fallback
    """
    ...
    if engine is not None and pl_name is not None:
        try:
            result = compute_inttime_with_ref_star(
                pl_name=pl_name, band=band, engine=engine,
                ref_star_name=ref_star_name, R=R_val,
...
def interpolate_target_data(
    ...
    t_wall_hrs = (1.0 + R_val) * t_sci_hrs
...
# ===========================================================================
# SECTION 7 — OBSERVABILITY AND WINDOW UTILITIES
# ===========================================================================
...
# ===========================================================================
# SECTION 9 — LEVEL 1: SEQUENCE SCORING
# ===========================================================================
...
def score_all_sequences(
    ...
    for seq in permutations(sci_keys):
        ...
        total_wall_s = compute_wall_clock_s(
            tint_s=tint_s, slew_inter_s=slew_inter_s,
            intra_slew=intra_slew, sci_keys=sci_keys, adi=adi,
        )
        results.append({
            "sequence":             seq,
...
# ===========================================================================
# SECTION 10 — COMBINATION SCORING
# ===========================================================================
...
def score_combination(
    ...
    return {
        ...
        "R_per_target":          {sci_to_ref[k]: tint_s[k]["R"] for k in sci_keys},
    }


# ===========================================================================
# SECTION 11 — LEVEL 2: MISSION KNAPSACK SCHEDULER
# ===========================================================================
...
def print_optimal_sequence(
    ...
    """Print the Pareto-optimal sequence with full time breakdown."""
    best        = None
    best_median = np.inf
    ...
    if matched:
        ...
        for k in seq:
            ...
            print(f"    Reference ({obj_meta[k]['name']}, R={R_k:.3f})"
                  f"{'':>10} R×T_sci = {R_k * sci_s / 3600 / len(seq):.2f} hrs")
...
def print_sequence_time_table(
    ...
    """Print wall-clock breakdown for every permutation."""
...
def print_mission_schedule(schedule: MissionSchedule) -> None:
    """Print the full mission timeline table."""
    sep = "=" * 115
    print(f"\n{sep}")
...
def print_campaign_summary(combo_results: list, top_k: int = 10) -> None:
    """Print ranked campaign combinations."""
...
def plot_campaign_summary(combo_results: list) -> None:
    ...
    ax1.set_xlabel("Best wall-clock (hrs)")
    ax1.set_ylabel("Combined completeness")
...
def plot_R_sweep(result: dict, title: str = "") -> None:
    """Plot T_science and T_wall vs R — matches notebook 07 figure."""
...
def run_campaign(
    ...
    Level 2: Greedy or exact knapsack selects campaigns within budget.

    Key change vs old code: CorgietcSession is instantiated once per band
    and reused across all targets (no per-target cache-wipe or module reload).
    """
    ...
    r_status = (
        "provided" if R_map
        else ("auto-computed via optimise_R" if engine else f"none — fallback R={_R_FALLBACK}")
    )
    print(f"  R_map: {r_status}")
    print(f"{'='*65}")
    ...
    if engine is not None and R_map is None:
        ...
        for name in valid_targets:
            ...
            if pl is None:
                ...
                if keys:
                    star_part = keys[0][0].lstrip("* ").strip()
                    pl = f"{star_part} {keys[0][1]}"
            ...
            try:
                res = optimise_R(
                    pl_name=pl, band=band, engine=engine,
                    ref_star_name=ref_name, ref_star_spectype=ref_spectype,
                )
                R_map[name] = res["R_opt"]
            except Exception as e:
...
# ===========================================================================
# SECTION 15 — TOY CAMPAIGN (fast sanity check)
# ===========================================================================
...
if __name__ == "__main__":
    ...
    _parser.add_argument("--username", default="plandb_user", help="MySQL username for plandb")
    _args = _parser.parse_args()
    from corgidb.ingest import gen_engine as _gen_engine
    _engine = _gen_engine(_args.username)
    run_campaign(engine=_engine)